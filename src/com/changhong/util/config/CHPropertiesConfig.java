...
import com.changhong.common.CHReflectUtils;

import android.content.Context;

/**
 * @Title CHPropertiesConfig
 * @Package com.changhong.util.config
 * @Description CHPropertiesConfig是ThinkAndroid的Properties类型配置文件操作类
 * @version V1.0
 */
public class CHPropertiesConfig implements CHIConfig
{
	/** assets中配置信息文件 */
	private String assetsPath = "/assets/tacdonfig.properties";
	/** 软件Files文件夹中配置信息文件 */
	private String filesPath = "tacdonfig.properties";
	private static CHPropertiesConfig mPropertiesConfig;
	private static final String LOADFLAG = "assetsload";
	private Context mContext;
	private Properties mProperties;
	private CHPropertiesConfig(Context context)
	{
	 ...
	 * @return
	 */
	public static CHPropertiesConfig getPropertiesConfig(Context context)
	{
		if (mPropertiesConfig == null)
		{
			mPropertiesConfig = new CHPropertiesConfig(context);
		}
	...
	public void loadConfig() {
		...
		InputStream in = null;// = CHPropertiesConfig.class.getResourceAsStream(assetsPath);
		try {
			in = mContext.getAssets().open(filesPath);
			if (in != null) {
				props.load(in);
				...
				if (e.hasMoreElements()) {
					while (e.hasMoreElements()) {
						...
						compareMethod(key, props.getProperty(key));
						props.setProperty(key, props.getProperty(key));
					}
				}
			}
//cxp test			setBoolean(LOADFLAG, true);
	...
	//quxy add for compare the key with the Method of Class MyProperties and invoke the Method of the same name as key
	private void compareMethod(String key, String value) {
		MyProperties myProperties = MyProperties.getMyProperties();
		Method[] mMethod = myProperties.getClass().getDeclaredMethods();
		for (int i = 0; i < mMethod.length; i++) {
			if ((mMethod[i].getName()).equals("set" + key)) {
				try {
					mMethod[i].invoke(myProperties, value);
	...
	{
//cxp test		return getBoolean(LOADFLAG, false);
		return false;
	}
	...
	public void setConfig(Object entity)
	{
		...
		for (Field field : fields)
		{
			...
			if (!CHReflectUtils.isTransient(field))
			{
				if (CHReflectUtils.isBaseDateType(field))
				{
					String columnName = CHReflectUtils.getFieldName(field);
