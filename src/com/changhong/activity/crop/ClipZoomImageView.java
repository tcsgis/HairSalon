...

{
	...
	private static float SCALE_MID = 0.6f;
	private static float SCALE_INIT = 1.0f;
	...
	public ClipZoomImageView(Context context, int width, int height)
	{
		this(context, null);
		mCropWidth = width;
		mCropHeight = height;
...
//		if (mGestureDetector.onTouchEvent(event)) //llw 双击缩放
//			return true;
		mScaleGestureDetector.onTouchEvent(event); //llw 手势缩放
	...
	public void onGlobalLayout()
	{
		...
		//init padding
		mCropWidth = getWidth() > mCropWidth ? mCropWidth : getWidth();
		mCropHeight = getHeight() > mCropHeight ? mCropHeight : getHeight();
		mHorizontalPadding = (getWidth() - mCropWidth) / 2;
		...
		if (once)
		{
			...
			if(dw >= mCropWidth && dh >= mCropHeight){
				float w = (float)(mCropWidth) / dw;
				float h = (float)(mCropHeight) / dh;
				//允许缩小
				SCALE_MIN = Math.max(w, h);
			}
			else{
				if (dw < mCropWidth && dh > mCropHeight)
				{
					scale = (float)(mCropWidth) / dw;
				}
				
				if (dh < mCropHeight && dw > mCropWidth)
				{
					scale = (float)(mCropHeight) / dh;
				}
				
				if (dw < mCropWidth && dh < mCropHeight)
				{
					float scaleW = (float)(mCropWidth) / dw;
					float scaleH = (float)(mCropHeight) / dh;
			...
			if (lastWidth  != 0 && lastHeight != 0
				...
				mScaleMatrix.postTranslate((width - lastWidth) / 2, (height - lastHeight) / 2);
			}
		}
		lastWidth = width;
		lastHeight = height;
	...
	public Bitmap clip()
	{
		...
		return Bitmap.createBitmap(bitmap, mHorizontalPadding,
				mVerticalPadding, mCropWidth, mCropHeight);
	}
