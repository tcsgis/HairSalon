 ...
 * @since 2013-12-26
 */
public class WheelScrollableView extends WheelBaseView {
    private static final int MSG_SCROLL_STEP = 1;
    private static final int MSG_SCROLL_END = 2;
    /** 滚动状态 */
    protected WheelScrollState mScrollState;
    /** 动作步列表 */
    private LinkedList<WheelStep> mStepList;
    ...
    public WheelScrollableView(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
        mScrollState = new WheelScrollState(context);
    }

    public WheelScrollableView(Context context, AttributeSet attrs) {
        super(context, attrs);
        mScrollState = new WheelScrollState(context);
    }

    public WheelScrollableView(Context context) {
        super(context);
        mScrollState = new WheelScrollState(context);
    }
    ...
    protected void onInit() {
        super.onInit();
        mStepList = new LinkedList<WheelStep>();
    }
    ...
    protected void setInterpolator(Context context,Interpolator interpolator) {
        mScrollState = new WheelScrollState(context,interpolator);
    }
...
//        LogUtil.d(getLogTag(), "smoothScrollTo getCurrX : " + mScrollState.getCurrX());
    ...
    public void smoothScrollTo(int finalX, int finalY) {
        smoothScrollTo(finalX, finalY, WheelStep.DEFAULT_DURATION);
    }
    ...
    public void smoothScrollBy(int distanceX, int distanceY) {
        ...
        stopScroll();
        /* 设置滚动参数 */
        mScrollState.setScrollParams(distanceX, distanceY, WheelStep.DEFAULT_DURATION);
        /* 启动滚动 */
        startScroll();
    }
...
//        LogUtil.d(getLogTag(), "onScrollEnd");
        if ((mScrollListener != null && !touching && touched)||(mScrollListener != null && !touching && !touched)) {
//            LogUtil.d(getLogTag(), "onScrollEnd callback");
     ...
     * @param stepList
     */
    public void move(LinkedList<WheelStep> stepList) {
        stopCurrentMovement();
        mStepList.clear();
    ...
    private void actStep() {
        if (CollectionUtils.isEmpty(mStepList))
            return;
        WheelStep step = mStepList.removeFirst();
    ...
    private void scrollDistanceStep(WheelStep step) {
        smoothScrollBy(step.getDistanceX(), step.getDistanceY(), step.getDuration());
    }

    private void scrollFinalStep(WheelStep step) {
