...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
	...
	/** llw 
	 * 当加载完，有新的数据时，listview应该在smoothScroll完成后再刷新
	 */
	public void customResetRefresh(OnSmoothScrollFinishedListener listener){
		if (isRefreshing()) {
			setState(State.RESET);
			mState = State.RESET;
			...
			mHeaderLayout.reset();
			mFooterLayout.reset();
			smoothScrollTo(0, listener);//llw 只替换了这一步，其他流程不变
