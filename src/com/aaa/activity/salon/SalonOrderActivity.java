...
public class SalonOrderActivity extends LlwTitleActivity{
	...
	protected void onAfterOnCreate(Bundle savedInstanceState) {
		...
		setTitile(R.string.salon_orString1);
		doGetDatas();
	}
		...
		orderService.getMyOrders(new AsyncResponseCompletedHandler<String>() {
			...
			public String doCompleted(ResponseBean<?> response,
				...
				if(response.getState() >= 0 && response.getData() != null){
					...
					adapter = new MyAdapter(SalonOrderActivity.this, datas, getCHApplication());
						...
						if(response.getState() >= 0){
							...
							if(order.getFreeBarberId() != 0){
								if(order.getOrderStatus() == OrderStatus.Pending){
									adapter.setStatus(orderId, OrderStatus.Wating2);
								}
								else if(order.getOrderStatus() == OrderStatus.Wating1){
	...
	private void doCoupon(final int customId){
		...
		.setOKClick(R.string.barber_orString21, new View.OnClickListener() {
			...
			@Override
			public void onClick(View arg0) {
				Intent intent = new Intent(SalonOrderActivity.this, SendCouponActivity.class);
	...
	private class MyAdapter extends BaseAdapter{
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if(convertView == null){
				vh = new ViewHolder();
				convertView = LayoutInflater.from(context).inflate(R.layout.item_salon_order, null);
				vh.date = (TextView) convertView.findViewById(R.id.date);
				vh.time = (TextView) convertView.findViewById(R.id.time);
				vh.barber = (TextView) convertView.findViewById(R.id.barber);
				...
				vh.barber_type = (TextView) convertView.findViewById(R.id.barber_type);
			...
			try {
				...
				vh.barber.setText(item.getBarberName());
				vh.custom.setText(item.getCustomName());
				vh.extra_msg.setText(item.getDesc());
				vh.barber_type.setText(item.getFreeBarberId() != 0 ? R.string.salon_orString3 : R.string.salon_orString2);
				
				if(item.getFreeBarberId() != 0 || item.getValue() <= 0){
...
//					vh.delete.setVisibility(View.VISIBLE);
					vh.accept.setVisibility(View.INVISIBLE);
					vh.reject.setVisibility(View.INVISIBLE);
					vh.coupon.setVisibility(item.getFreeBarberId() != 0 ? View.GONE : View.VISIBLE);
					vh.state.setVisibility(View.VISIBLE);
					vh.state.setText(R.string.barber_orString10);
					break;
...
//					vh.delete.setVisibility(View.VISIBLE);
	...
	private void toast(int id){
		Toast.makeText(SalonOrderActivity.this, id, Toast.LENGTH_SHORT).show();
	}
}
