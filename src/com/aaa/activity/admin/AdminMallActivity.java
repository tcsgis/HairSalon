...
public class AdminMallActivity extends LlwTitleActivity{
	...
	private final int MALL_PHOTO_MAX = 3;
	...
	protected void onAfterOnCreate(Bundle savedInstanceState) {
		...
		setTitile(R.string.admin_maString1);
		setRightBtn(true, 0);
		...
		accountService.getBanners(new AsyncResponseCompletedHandler<String>() {
			...
			public String doCompleted(ResponseBean<?> response, ChCareWepApiServiceType servieType) {
				...
				if(response.getState() >= 0 && response.getData() != null){
					...
					photos = banners.get(1).getPhotos();
					urls = banners.get(1).getUrls();
					
					adapter = new PhotoAdapter(photos, AdminMallActivity.this);
					list.setAdapter(adapter);
				}else{
					Toast.makeText(AdminMallActivity.this, R.string.get_failed, Toast.LENGTH_SHORT).show();
				}
				hideAllDialog();
				return null;
			}
	...
	private void uploadBanners(ArrayList<String> newPhotos){
		...
		adminService.updateMallPics(bp, new AsyncResponseCompletedHandler<String>() {
			...
			public String doCompleted(ResponseBean<?> response,
				...
				if(response.getState() >= 0){
					Toast.makeText(AdminMallActivity.this, R.string.admin_adString3, Toast.LENGTH_SHORT).show();
					hideAllDialog();
					finish();
				}else{
					Toast.makeText(AdminMallActivity.this, R.string.admin_adString4, Toast.LENGTH_SHORT).show();
					hideAllDialog();
				}
				return null;
			}
	...
	private class PhotoAdapter extends BaseAdapter{
		...
		public PhotoAdapter(ArrayList<String> list, Context context) {
			...
			if(this.list.size() < MALL_PHOTO_MAX){
				this.list.add(null);
			}
		}
		...
		private void addPhoto(String path){
			if(list.size() == MALL_PHOTO_MAX){
				list.remove(MALL_PHOTO_MAX - 1);
				list.add(path);
			}else if(list.size() < MALL_PHOTO_MAX){
